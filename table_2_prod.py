...
def main():
    ...
    data = client2.query(script).result_set
    logging.info(data)
    ...
    for i in data:
        client1.command(f"""INSERT INTO {table_name} (region, city, id, month_unique, week_unique, day_unique)
                                 VALUES ('{i[1]}', '{i[2]}', '{i[3]}', '{i[6]}', '{i[5]}', '{i[4]}')""")
